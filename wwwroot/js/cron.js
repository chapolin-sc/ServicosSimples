...
;var vStopwatch = (function() {
    ...
    var Q = function() {
        ...
        w.click(function() {
            hideTooltips();
            if (configC.swFontSizeId < FONT_SIZE_LEN && M == FONT_SIZES[configC.swFontSizeId]) {
                configC.swFontSizeId++
            }
            configC.save("swFontSizeId");
            y()
        ...
        var ab = 0;
        var aa = $(ae(19377203) + "[" + ae(19446771) + "=" + ae(1527707154097768) + "]").attr(ae(1057973211936));
        if (aa === undefined) {
            return
        }
    ...
    function E() {
        ...
        if (J.laps.length == 0) {
            ...
            for (var aa = 0; aa < J.laps.length; aa++) {
                ab = "<tr class='digit'><td>" + (aa + 1) + "</td><td>" + z(aa == 0 ? J.laps[aa] : J.laps[aa] - J.laps[aa - 1]) + "</td><td>" + z(J.laps[aa]) + "</td></tr>" + ab
            }
            ab = "<tr class='digit'><td>" + (J.laps.length + 1) + "</td><td id='lbl-lap-time'>" + z(J.value - n) + "</td><td id='lbl-lap-total'>" + z(J.value) + "</td></tr>" + ab
        }
        A.html(a + ab);
        V = $("#lbl-lap-time");
        S = $("#lbl-lap-total");
    ...
    function P(ab) {
        ...
        if (J.title != "") {
            K.css("font-size", Math.max(Math.round(M / 4), MIN_TITLE_FONT_SIZE) + "px")
        }
        var aa = Math.max(Math.round(M / 6), 14);
        A.css("font-size", aa + "px")
    }
    function W() {
        return Y.height() + (J.title != "" ? K.height() + B + T : T) + T + H.height() + B
    }
    function f() {
    ...
    function y() {
        ...
        var ad = p + s;
        var ac = ad + s + (J.title != "" ? MIN_LABEL_HEIGHT : 0) + (configC.clockDateVisible ? MIN_LABEL_HEIGHT : 0) + H.height() + MIN_TIME_HEIGHT;
        var al = FONT_SIZES[configC.swFontSizeId];
        var aj = Math.max(ac, $(window).height() - u.offset().top - (isEmbed || t.length === 0 ? 0 : 90) - Math.max(MIN_BOTTOM_MARGIN, U.position().top));
        function af() {
            if (j.is(":visible")) {
                u.css("height", Math.round(((aj + Y.height()) / 2) + H.height() * 2) + "px")
            } else {
                u.css("height", aj + "px")
            }
        }
        ...
        i.css("left", (langC.rtl ? -1 : 1) * Math.round((u.width() - Y.width()) / 2) + "px");
        var ag = Math.round(((aj - i.height()) / 2));
        if ((u.height() < W() + H.height() + (J.title == "" ? B : 0))) {
            i.css("top", s + (J.title != "" ? K.height() + B + T : T) + Math.round(((u.height() - W()) / 2) - Y.position().top) + "px")
        } else {
        ...
        H.css("top", i.position().top + Y.position().top + Y.height() + Math.round(H.height() / 2) + "px")
    }
    function O() {
        var aa = (M <= FONT_SIZES[0]);
        var ab = (configC.swFontSizeId == FONT_SIZE_LEN || M < FONT_SIZES[configC.swFontSizeId]);
        if (aa && ab) {
            N.hide();
            w.hide()
        } else {
    ...
    function h(aa) {
        ...
        if (ak != 0) {
            ...
            if (configC.themeDigitalFont) {
                al = ak + "<span class='" + (langC.isDigitalDay ? "font-digit-text text-rel-30'>" : "font-sans text-normal text-rel-40'>&nbsp;") + ag + "</span>&nbsp;" + al
            } else {
                al = ak + "<span class='text-normal text-40'>&nbsp;" + ag + "</span>&nbsp;" + al
            }
        }
        if (ab) {
            document.title = (J.value != 0 ? ae + " - " : "") + (J.title == "" ? ORIGINAL_TITLE : J.title)
        }
        if (langC.rtl) {
            Y.html(al + d(ac))
        } else {
    ...
    function z(ae) {
        ...
        if (ah != 0) {
            ...
            if (configC.themeDigitalFont) {
                ad = ah + (ac.charAt(0) == "d" ? "d " : "<span class='font-sans text-normal text-rel-70'> " + ac + "</span> ") + ad
            } else {
